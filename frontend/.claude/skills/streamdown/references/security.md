...
Use the `urlTransform` prop for custom URL rewriting. The default `defaultUrlTransform` is a passthrough — URL security
is handled by `rehype-sanitize` and `rehype-harden`.
